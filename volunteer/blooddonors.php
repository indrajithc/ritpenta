 ...
 * @Last Modified time: 2018-11-17 23:14:54
 */ 
...
if(isset($_POST['submit'])){
	...
	$stmnt='select * from nss_blood_donation where bd_admno= :bd_admno AND DATE(bd_date) = :date';
	...
	if(!$db->display($stmnt,$params)){
		...
		$params=array(
			...
			'bd_mobile'            =>  $vol_mob,
			'bd_quantity'         =>  $vol_ml,
			'bg_added_by'         =>  $_SESSION[SYSTEM_NAME . 'userid0']
		...
		<div class="table-responsive">
			...
			<table class="table dataTable table-hover bg-white">
				...
				<tbody>
					...
					<?php
					$stmnt=" SELECT v.*, d.branch_or_specialisation, d.name, DATE(v.bd_date) AS ddate FROM `nss_blood_donation` v LEFT JOIN  stud_details d ON v.bd_admno = d.admissionno  ";
